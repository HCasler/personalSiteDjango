...
class Video(models.Model):
    def __str__(self):
        return "Video {0}: {1}".format(self.id, self.title)
...
class Playlist(models.Model):
    ...
    def __str__(self):
        if self.playlistType == Playlist_Type.VIDEO:
            return "Video {0}: {1} playlist id {2}, bandwidth {3}".format(self.video, self.playlistType, self.id, self.bandwidth)
        else:
            return "Video {0}: {1} playlist id {2}".format(self.video, self.playlistType, self.id)
...
class MediaSegment(models.Model):
    def __str__(self):
        return "Media segment {0} from playlist id {1}".format(self.segmentNum, self.playlist)
